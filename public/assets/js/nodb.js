...
function NOdb(config=null){
    ...
    function pure(str){
        let new_str = [];
        let keywords = ["CREATE","TABLE","DROP","COLUMN","SET","UPDATE","INSERT","INTO","DELETE","TRUNCATE","SELECT","FROM","WHERE","ORDER","BY","LIMIT","ASC","DESC","NOT","AND","OR","VALUES","VALUE","DISTINCT","ALTER","ADD","AFTER","JOIN","IF","EXIST"];
        str = str.trim().split(" ");
    ...
    this.query = function(qry){
        ...
        if(word == "CREATE"){
            ...
            if(str[1].toUpperCase() == "TABLE"){
                ...
                if(line.endsWith("if not exist")){
                    line = line.substring(0,line.indexOf("if not exist"));
                    no_error = true;
                }
                ...
                if(rows.length != 0){
                    ...
                    nrows.forEach(function(i){
                        ...
                        try{
                            ...
                            throw("Bug: Caused by adding , to values");
                        }
                    });
                }
